...
class QuoteDownloader:
    ...
    def __init__(self, logger: logging.Logger, loop: asyncio.AbstractEventLoop) -> None:
        ...
        self.token = token.Token.load("snergebot")
    ...
    async def communication(self) -> None:
        with open("output.txt", "a", encoding="utf-8") as file:
            async with self.session.ws_connect("wss://eventsub.wss.twitch.tv/ws?keepalive_timeout_seconds=60") as socket:
                welcome: aiohttp.WSMessage = await socket.__anext__()
                welcome_data = welcome.json()

                session_id = welcome_data["payload"]["session"]["id"]
                self.logger.info(f"Session ID: %s", session_id)
                ...
                async for msg in socket:
                    if msg.type != aiohttp.WSMsgType.TEXT:
                        self.logger.warning("Unexpected %s message type", msg.type, extra={"msg": msg})
                        continue
                    ...
                    if data["payload"]["subscription"]["type"] != "channel.chat.message":
                        continue
                    if data["payload"]["event"]["chatter_user_id"] == self.token.user_id:
                        continue
    ...
    async def send_message(self, message: str) -> None:
        response = await self.session.post(
            ...
            json={
                ...
                "sender_id": self.token.user_id,  # SnergeBot
                "message": message,
            }
        )
        ...
        response = await self.session.post(
            ...
            json={
                ...
                "condition": {
                    ...
                    "user_id": str(self.token.user_id)  # SnergeBot
                },
                "transport": {
                    "method": "websocket",
